...
df = pd.read_excel('boat1_2_sin.xlsx').iloc[Start_row:Start_row+row]
...
# 平滑滤波
u = savgol_filter(u, window_length=11, polyorder=1)
v = savgol_filter(v, window_length=11, polyorder=1)
r = savgol_filter(r, window_length=11, polyorder=1)
...
if result.success:
    ...
    a1, a2, a3, a4, a5, b1, b2, b3, b4, b5, c1, c2, c3, c4, c5,c6  = estimated_params

    # 打印模型方程
    print("\n辨识的模型方程:")
    print(f"{a1:.6f} * v * r + {a2:.6f} * u + {a3:.6f} * r + {a4:.6f} + {a5:.6f} * (Tp + Ts) ,")
    print(f"{b1:.6f} * u * r + {b2:.6f} * v + {b3:.6f} * r + {b4:.6f} + {b5:.6f} * (Tp - Ts) ,")
    print(f"{c1:.6f} * u * v + {c2:.6f} * v + {c3:.6f} * r + {c4:.6f} *u + {c5:.6f} * (Tp - Ts)+ {c6:.6f}  ,")
